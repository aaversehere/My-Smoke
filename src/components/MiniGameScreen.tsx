...
export const MiniGameScreen: React.FC<MiniGameScreenProps> = ({ onComplete }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-80px)] px-4">
      <div className="max-w-2xl w-full bg-zinc-900/50 backdrop-blur-md rounded-[2rem] border border-zinc-800 p-8 md:p-12 text-center shadow-2xl relative overflow-hidden">
        
        {!showResult ? (
          <div className="flex flex-col items-center animate-in fade-in zoom-in duration-500">
            <h2 className="font-['Montserrat'] font-bold text-2xl md:text-3xl text-emerald-400 mb-2">
              Choose Your Power
            </h2>
                ...
                <button
                  ...
                  className={`flex items-center gap-4 p-4 rounded-2xl border transition-all duration-300 ${
                    selectedPower === power.id
                      ...
                      : 'bg-zinc-800/50 border-zinc-700 hover:bg-zinc-800 hover:border-zinc-600 hover:scale-[1.02]'
                  }`}
                >
                  <span className="text-3xl">{power.icon}</span>
                  <span className="font-bold text-white font-['Quicksand']">{power.label}</span>
                </button>
              ))}
            </div>
          </div>
