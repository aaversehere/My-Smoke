    ...
    <div className="flex flex-col w-full gap-8 relative z-10 pb-16 pt-2 px-6 md:px-12 lg:px-24 min-h-[calc(100vh-80px)]">
      ...
      <div className="flex flex-col gap-4 w-full" id="mission-cards">
        ...
        <div 
          ...
          className={`mission-card relative bg-white dark:bg-zinc-900 rounded-xl p-5 shadow-lg transform transition-all duration-300 active:scale-95 cursor-pointer border-t-4 border-sage-500 border-x border-b border-zinc-200 dark:border-zinc-800 ${
            expandedCard === 1 ? 'ring-2 ring-sage-500' : ''
          }`}
        >
          <div className="flex items-start gap-4">
            <div className="flex-shrink-0 w-12 h-12 rounded-full bg-sage-500/10 border border-sage-500/20 flex items-center justify-center shadow-inner">
      ...
      <div className="mt-4 flex flex-col gap-3 items-center w-full">
        <button
          onClick={handleGotIt}
          disabled={completed}
          className={`w-full max-w-sm h-14 font-['Montserrat'] font-extrabold text-lg rounded-full shadow-[0_8px_20px_rgba(92,131,88,0.3)] flex items-center justify-center gap-2 transform transition-all duration-300 active:scale-95 overflow-hidden relative cursor-pointer ${
            completed
              ? 'bg-sage-400 text-black shadow-sm'
              : 'bg-sage-500 text-black hover:bg-sage-400'
          }`}
        >
          {completed ? (
            <>
