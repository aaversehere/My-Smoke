      ...
      case 0:
        return (
          <div className="flex flex-col items-center animate-in fade-in zoom-in duration-500">
            <h2 className="font-['Montserrat'] font-bold text-3xl md:text-4xl text-emerald-600 dark:text-emerald-400 mb-4">
            ...
            <div className="flex flex-col gap-3 w-full max-w-sm">
              {['15', '16', '17', '18', 'Other'].map((opt) => (
                <button
                  ...
                  className="bg-zinc-100 dark:bg-zinc-800/80 border border-zinc-300 dark:border-zinc-700 hover:border-emerald-500 hover:bg-zinc-100 dark:bg-zinc-800 text-zinc-200 p-4 rounded-xl font-bold transition-all hover:scale-[1.02]"
                >
                  {opt}
                </button>
              ))}
          ...
          <div className="flex flex-col items-center animate-in fade-in slide-in-from-right-4 duration-300 w-full">
            ...
            <div className="flex items-center gap-4 mb-8">
              <input 
                ...
                className="bg-white dark:bg-zinc-900 border-2 border-zinc-300 dark:border-zinc-700 focus:border-emerald-500 rounded-xl px-4 py-3 text-zinc-900 dark:text-white text-xl text-center w-24 outline-none transition-colors"
                placeholder="Age"
                min="5"
                max="30"
              />
            ...
            <div className="flex gap-4 w-full max-w-xs">
              {['Yes', 'No'].map((opt) => (
                <button
                  key={opt}
                  onClick={handleChoice}
                  className="flex-1 bg-zinc-100 dark:bg-zinc-800/80 border border-zinc-300 dark:border-zinc-700 hover:border-emerald-500 hover:bg-zinc-100 dark:bg-zinc-800 text-zinc-200 py-6 rounded-xl font-bold text-xl transition-all hover:scale-[1.02]"
                >
                  {opt}
                </button>
              ))}
      ...
      case 8:
        return (
          <div className="flex flex-col items-center animate-in fade-in slide-in-from-right-4 duration-500 w-full">
            <h2 className="font-['Montserrat'] font-bold text-3xl md:text-4xl text-emerald-600 dark:text-emerald-400 mb-4 flex items-center gap-3">
            ...
            <div className="w-full max-w-sm bg-zinc-100 dark:bg-zinc-800/50 rounded-xl p-6 mb-8 border border-zinc-300 dark:border-zinc-700">
              ...
              <p className="text-emerald-600 dark:text-emerald-400 text-2xl font-bold font-['Montserrat']">20% COMPLETE</p>
            </div>
            <button
              onClick={nextStep}
            ...
            {[1, 2, 3, 4, 5, 6, 7].map((s) => (
              <div 
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  s === step ? 'w-8 bg-emerald-400' : s < step ? 'w-2 bg-emerald-500/50' : 'w-2 bg-zinc-100 dark:bg-zinc-800'
                }`}
              />
            ))}
          </div>
