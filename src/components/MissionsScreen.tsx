...
export const MissionsScreen: React.FC<MissionsScreenProps> = ({ onComplete }) => {
  ...
  const renderQuizCard = (
    ...
    <div key={q.id} className="bg-zinc-100 dark:bg-zinc-800/50 p-4 rounded-xl border border-zinc-300 dark:border-zinc-700">
      ...
      {answers[q.id] !== null && (
        <div className={`text-sm font-bold animate-in fade-in ${answers[q.id] === true ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-400'}`}>
          {answers[q.id] === true 
  ...
  const renderMission1 = () => (
    <div className="flex flex-col items-center animate-in fade-in slide-in-from-right-4 duration-300 w-full">
      <h2 className="font-['Montserrat'] font-bold text-2xl md:text-3xl text-emerald-600 dark:text-emerald-400 mb-6 flex items-center justify-center gap-3 text-center">
  ...
  const renderMission5 = () => (
    <div className="flex flex-col items-center animate-in fade-in slide-in-from-right-4 duration-300 w-full">
      <h2 className="font-['Montserrat'] font-bold text-2xl md:text-3xl text-emerald-600 dark:text-emerald-400 mb-8 text-center flex flex-col items-center gap-2">
        Proactive Coping
      ...
      <div className="w-full max-w-lg mb-8 text-left bg-zinc-100 dark:bg-zinc-800/50 p-6 rounded-2xl border border-zinc-300 dark:border-zinc-700">
        <h3 className="text-emerald-600 dark:text-emerald-400 font-bold text-lg mb-2">If this happens...</h3>
        <p className="text-zinc-900 dark:text-white text-xl mb-6">My friends offer me a cigarette.</p>
        
        <h3 className="text-emerald-600 dark:text-emerald-400 font-bold text-lg mb-4">I will...</h3>
          ...
          {[1, 2, 3, 4, 5].map((m) => (
            <div 
              ...
              className={`h-2 rounded-full transition-all duration-300 ${
                m === mission ? 'w-10 bg-emerald-400' : m < mission ? 'w-3 bg-emerald-500/50' : 'w-3 bg-zinc-100 dark:bg-zinc-800'
              }`}
            />
          ))}
        </div>
