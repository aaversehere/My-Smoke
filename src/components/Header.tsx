...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 bg-[#09090b]/90 backdrop-blur-xl pt-safe border-b border-zinc-800/80 transition-all">
      {/* Primary Header Row */}
      <div className="h-14 px-6 flex items-center justify-between w-full">
        <button 
          onClick={() => onNavigate('welcome')}
          className="font-['Montserrat'] font-bold text-xl text-emerald-400 flex items-center gap-1.5 hover:opacity-90 transition-opacity"
        >
          SmokeFree Quest
        </button>

        <div className="flex items-center gap-2">
          <span className="font-['Montserrat'] font-bold text-[12px] uppercase text-emerald-400 tracking-wider bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1 rounded-full">
            {respondentId}
          </span>
          <button 
            onClick={() => onNavigate('snapshot')}
            title="View User Profile & Snapshot"
            className="w-8 h-8 rounded-full bg-emerald-500 text-black flex items-center justify-center font-bold shadow-md hover:scale-105 active:scale-95 transition-all"
          >
